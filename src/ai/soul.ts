...
interface SoulArchetypeDefinition {
  key: SoulArchetypeKey;
  name: string;
  summary: string;
  publicBlurb: string;
  traits: SoulTraits;
...
const SOUL_ARCHETYPES: SoulArchetypeDefinition[] = [
  {
    key: 'mathematician',
    name: '数学派',
...
export function generateSoulProfile(seed: number = Math.floor(Math.random() * 1_000_000)): SoulProfile {
...
export function describeSoulTraits(traits: SoulTraits): string[] {
  const descriptions: string[] = [];

  if (traits.aggression >= 70) descriptions.push('偏激进');
  if (traits.aggression <= 35) descriptions.push('偏克制');
  if (traits.riskTolerance >= 70) descriptions.push('风险偏好高');
  if (traits.riskTolerance <= 35) descriptions.push('风险偏好低');
  if (traits.deception >= 70) descriptions.push('擅长表演');
  if (traits.vengefulness >= 70) descriptions.push('容易记仇');
  if (traits.patience >= 70) descriptions.push('很有耐心');
  if (traits.emotionalVolatility >= 70) descriptions.push('情绪起伏大');
  if (traits.discipline >= 75) descriptions.push('纪律性强');

  return descriptions.length > 0 ? descriptions : ['风格微妙'];
}

export function buildSoulPrompt(profile: SoulProfile): string {
